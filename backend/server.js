...
// // Global cache for this customer's ingredients to avoid
// // Global cache for allergic ingredients - only computed once per server run
// let GLOBAL_ALLERGY_CACHE = {};
// let CUSTOMER_ALLERGIC_INGREDIENTS = null;

// // Add this near the top of your file where you set up the server
// const initializeAllergyCache = async () => {
//     try {
//         console.log('🔄 Pre-loading all allergies...');

//         // Fetch all allergy records at once
//         const allAllergies = await base('Allergies Diet').select().all();

//         // Build the cache
//         allAllergies.forEach(record => {
//             const allergyName = record.fields['Allergy to/ (As) Diet Type'] || 'Unknown';
//             const ingredientIds = record.fields['Ingredient to Avoid'] || [];

//             GLOBAL_ALLERGY_CACHE[record.id] = {
//                 name: allergyName,
//                 ingredientIds: ingredientIds
//             };
//         });

//         console.log(`🔄 Pre-loaded ${Object.keys(GLOBAL_ALLERGY_CACHE).length} allergies`);
//     } catch (error) {
//         console.error('❌ Error pre-loading allergies:', error);
//     }
// };

// // Call this when the server starts
// initializeAllergyCache();

// // Function to get ingredients to avoid (runs once per page load)
// const getIngredientsToAvoid = async (customerAllergies) => {
//     // If we already processed this customer's allergies, return the cached result
//     if (CUSTOMER_ALLERGIC_INGREDIENTS !== null) {
//         console.log('🔍 Using cached ingredients to avoid list');
//         return CUSTOMER_ALLERGIC_INGREDIENTS;
//     }

//     console.log('🔍 Processing customer allergies:', customerAllergies);
...
//     const ingredientsToAvoid = new Set();

//     // Process each customer allergy
//     for (const allergyId of customerAllergies) {
//         // Check if this allergy is already in the global cache
//         if (GLOBAL_ALLERGY_CACHE[allergyId]) {
//             console.log(`🔍 Using cached allergy: ${GLOBAL_ALLERGY_CACHE[allergyId].name}`);

//             // Add all linked ingredients from cache
//             GLOBAL_ALLERGY_CACHE[allergyId].ingredientIds.forEach(id => {
//                 ingredientsToAvoid.add(id);
//             });

//             continue;
//         }

//         // Fetch allergy record if not in cache
//         try {
//             const allergyRecord = await base('Allergies Diet').find(allergyId);
//             const allergyName = allergyRecord.fields['Allergy to/ (As) Diet Type'] || 'Unknown';
//             console.log(`🔍 Processing new allergy: ${allergyName}`);

//             const linkedIngredients = allergyRecord.fields['Ingredient to Avoid'] || [];

//             // Store in global cache
//             GLOBAL_ALLERGY_CACHE[allergyId] = {
//                 name: allergyName,
//                 ingredientIds: linkedIngredients
//             };

//             // Add to ingredients to avoid
//             linkedIngredients.forEach(id => ingredientsToAvoid.add(id));

//         } catch (err) {
//             console.warn(`⚠️ Could not find allergy record: ${allergyId}`);
//         }
//     }

//     // Special case for string beans
//     console.log('🔍 Adding string beans to ingredients to avoid');
//     ingredientsToAvoid.add('11053'); // Add string beans ID

//     // Cache the result for this customer
//     CUSTOMER_ALLERGIC_INGREDIENTS = Array.from(ingredientsToAvoid);

//     console.log(`🔍 Processed allergies, found ${CUSTOMER_ALLERGIC_INGREDIENTS.length} ingredients to avoid`);

//     return CUSTOMER_ALLERGIC_INGREDIENTS;
// };

// const getSauceAndGarnishOptions = async (allIngredientIds, ingredientNames, ingredientComponents, currentIngredientIds, originalIngredientIds, mealType, customerAllergies = []) => {

//     console.log(`🥕 Getting veggie/starch options for meal type: ${mealType}`);

//     // Get the cached ingredients to avoid
//     const ingredientsToAvoid = await getIngredientsToAvoid(customerAllergies);

//     //console.log('🍶 Getting sauce/garnish options for', allIngredientIds.length, 'ingredients');

//     console.log('🚫 Customer allergies passed to getSauceAndGarnishOptions:', customerAllergies);
...
//     const veggieStarchOptions = await getVeggieStarchOptions(allIngredientIds, ingredientNames, ingredientComponents, currentIngredientIds, originalIngredientIds, mealType, customerAllergies);
...
// const getVeggieStarchOptions = async (allIngredientIds, ingredientNames, ingredientComponents, currentIngredientIds, originalIngredientIds, mealType, customerAllergies = []) => {

//     console.log('🥕 Getting veggie/starch options for meal type:', mealType);
//     console.log('🚫 Customer allergies:', customerAllergies);

//     // First, we need to get the list of ingredients to avoid based on allergies
//     const ingredientsToAvoid = await getIngredientsToAvoid(customerAllergies);
//     console.log(`🚫 Found ${ingredientsToAvoid.length} ingredients to avoid for this customer`);

//     const stringBeansKeywords = ['string bean', 'string beans', 'green bean', 'green beans'];
...
//     // Filter out allergic ingredients
//     const originalVeggies = originalIngredientIds
//         .filter(id => {
//             // Keep only Veggies that aren't already in current selection
//             // AND that aren't in the allergen list
//             return ingredientComponents[id] === 'Veggies' &&
//                 !currentIngredientIds.includes(id) &&
//                 !ingredientsToAvoid.includes(id);
//         })
//         .map(id => ({
//             id,
//             name: ingredientNames[id] || id,
...
//     console.log(`🥕 Original veggies after allergy filtering: ${originalVeggies.length}`);
...
//             // Skip if this ingredient is already in original veggies
//             if (originalIngredientIds.includes(ingredientId)) continue;

//             // Skip if customer is allergic to this ingredient
//             if (ingredientsToAvoid.includes(ingredientId)) {
//                 console.log('🚫 SKIPPING allergic ingredient:', ingredientNames[ingredientId]);
//                 continue;
//             }
...
//     console.log(`🥕 Final veggie options after allergy filtering: ${finalVeggieOptions.length}`);
...
//         .filter(id => {
//             return ingredientComponents[id] === 'Starch' &&
//                 !currentIngredientIds.includes(id) &&
//                 !ingredientsToAvoid.includes(id);
//         })
//         .map(id => ({
...
//         for (const ingredientId of variantIngredients) {
//             if (ingredientsToAvoid.includes(ingredientId)) {
//                 console.log('🚫 SKIPPING allergic starch:', ingredientNames[ingredientId]);
//                 continue;
//             }
...
//     // Add this filtering step for string beans by name
//     const finalFilteredVeggieOptions = finalVeggieOptions.filter(veggie => {
//         if (!veggie.name) return true;

//         const veggieName = veggie.name.toLowerCase();
//         const isStringBeans = stringBeansKeywords.some(keyword => veggieName.includes(keyword));

//         if (isStringBeans) {
//             console.log(`🚫 Filtering out string beans by name: ${veggie.name}`);
//             return false;
//         }
...
//     console.log(`🥕 Final veggie options: ${finalFilteredVeggieOptions.length}`);

//     return {
//         veggieOptions: finalFilteredVeggieOptions,
...
// // Add to your API routes to reset the cache during development
// app.get('/api/reset-cache', (req, res) => {
//     GLOBAL_ALLERGY_CACHE = {};
//     CUSTOMER_ALLERGIC_INGREDIENTS = null;
//     console.log('🔄 Allergy caches reset');
...
//                 console.log('🔍 About to pass allergies:', clientProfile.fields['Allergies_Diet']);

//                 sauceAndGarnishOptions = await getSauceAndGarnishOptions(allIngredientIds, ingredientNames, ingredientComponents, currentIngredientIds, originalIngredientIds, mealType, clientProfile.fields['Allergies_Diet'] || []
//                 );
...
// but BEFORE any routes or other middleware
app.use((req, res, next) => {
...
app.get('/api/orders/:token', async (req, res) => {
    ...
    try {
        ...
        // Right before sending the response, log its size
        const responseSize = JSON.stringify(response).length;
        console.log(`Sending response of size: ${responseSize} bytes`);
        ...
        const simpleResponse = {
            ...
            message: 'Found data but returning simplified response for testing'
        };
